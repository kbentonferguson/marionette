...
stdlib-only backend (urllib + sqlite); `puppetmaster-ai==1.22.48` is the one
real dependency the installer puts in the venv.

> Status: v0.9.420, deliberately pre-1.0. Rides puppetmaster-ai==1.22.48. A silent provider stream aborts at the 5m stuck line with Continue/Retry instead of hanging on Waiting. The wait clock floors to this busy period so a leftover idle stamp cannot show 44s on a 2s send. An empty mid-switch session refresh cannot wipe last-good rows. Stop stays Stopped: a later assistant_done cannot paint Done over Continue. Apply is not acceptance — a landed implement without proof reserves diagnosis tools and refuses another paid swarm until the user continues (Benton #323). Session click paints one active row and hydrates immediately; it does not refetch every project list or dim the rail. BRANCHES uses a fixed height so the handle is the pane edge. The exposed worker catalog is the Settings-enabled set (so `gpt-6-astra` is pin-visible), not the first 16 file-order rows. A successful implement apply completes one uniquely matching composer TODO wave; validate-gate and UI/server-action items stay open unless the job names that surface. Settings enable still writes the isolated worker catalog; `gpt-5.6-astra` pins map to enabled `gpt-6-astra`, not Sol. A provider 400 that says reasoning cannot be disabled is retried once with the disable omitted (GPT-5 tool turns still send `none` first). Swarm Tracker Session scope resolves the unscoped active session during the pre-`harness-project-selected` window so a live `/api/swarm/live` row does not vanish (Benton #313). Settings and composer Workers knobs (factory medium) stamp swarm, implement, and parallel payloads; the composer picker stays the chat pilot. Electron backend spawn pins the Marionette catalog and drops inherited Puppetmaster adapter/provider policy; a normal launch does not relocate the job store into harness state. `scripts/dev.sh` stays the replace launcher; `scripts/inspect.sh` is the supported side-by-side fixture (isolated state/userData, no credential fold, no production backend marker). Worker patches land in the validated target repo, including `run_parallel`. Package scripts pin `NODE_ENV` so a production parent shell cannot empty the dev renderer or trip React `act`. 1.22.48 defaults every swarm adapter to medium reasoning unless the caller pinned one.
...
| **Puppetmaster delegation** | run_swarm (read-only analysis), run_implement (edit-capable worktree worker), run_parallel (concurrent waves). Heavy/multi-file work runs as durable, auditable jobs. Requires `puppetmaster-ai==1.22.48` (1.22.48 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#133 -- every swarm adapter defaults to medium reasoning, caller pins win; 1.22.47 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#131/#132 -- OpenRouter-class only; headed Chrome auth handoff with a durable profile and shared CDP port, per-worker isolate profiles when `PM_BROWSER_ISOLATE_KEY` is set, one-shot host SCM observe (`observe-scm` / `puppetmaster_observe_scm`), configurable default reviewer platform, shared `build_cost_report`, safely resumable jobs, honest delivery verdicts, Google Antigravity `agy` adapter, dashboard per-project isolation, hermetic test-suite credential isolation, swarm timeout propagation, Sonnet 5 catalogs, discovery origin, spawned-worker delegate-first exemption, SWE-bench Bash Only priors, exact registry pins, and Bedrock invoke health. 1.22.44 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#126 -- host.started vs host.recovered plus lifecycle-default job event reads; 1.22.43 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#125 -- fail-closed nested job starts inside workers (`PUPPETMASTER_WORKER=1`; job-start CLI/MCP refused unless `PUPPETMASTER_ALLOW_NESTED=1`); 1.22.42 -- PyPI live, GitHub issue professorpalmer/Puppetmaster#122 / PR professorpalmer/Puppetmaster#123 -- stable terminal completed-job cost receipts, unknown selected-model cost remains null (no fake savings), reset generations refreeze; 1.22.41 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#121 -- same-turn GATE before follow-ups, live-lease writer fence on claim, listing is still not a board; 1.22.40 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#119 -- supervisor-only SQLite schema, workers `attach()` instead of racing `init()`, no worker `recover_stale_tasks` herd; 1.22.39 -- PyPI live, GitHub PR professorpalmer/Puppetmaster#116 -- cited-artifact freshness at inject, FINDING shared context as last-wins handles, skip re-enqueue of the same failed GATE/rejected gist/CI failure; 1.22.38 host SCM observe plus per-worker browser isolate; 1.22.37 METR seams remain: graph is the only dispatcher (same-job children; foreign job / new job / wrong parent refused; worker recruit/HOLD/VETO/mailbox is `task.enqueue_refused`); no writable listing API across jobs (protocol gists rejected; cross-job inject needs `independently_supported`); coordinator outlives workers (job `acceptance_criteria` / `granted_authority` / `wait_reason` `waiting_external` vs `waiting_user`; failed GATE does not enqueue merge/ship; worker cannot `update_job_status` to COMPLETE); host receipts beat worker shipped/merged/released claims (idempotent `record_host_observation`); one writer per subgraph (leases + HOLD/VETO/`subgraph_owner`)). |
