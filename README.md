...
> Status: v0.9.421, deliberately pre-1.0. Rides puppetmaster-ai==1.22.48. Session switches preserve drafts and reading position, reject stale transcript responses, and recover failed cold loads. Goals persist per session, including migration of existing goals. Hidden panels stop redundant polling. Acceptance checks distinguish missing evidence from demonstrated failures, bound repeated corrections, and identify automatic goal messages as system controls. Silent-stream cancellation and patch-acceptance reporting from v0.9.420 are retained.
