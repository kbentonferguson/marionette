...
stdlib-only backend (urllib + sqlite); `puppetmaster-ai==1.27.13` is the one
real dependency the installer puts in the venv.

> Status: v0.9.477, deliberately pre-1.0. Pin is `puppetmaster-ai==1.27.13` (PyPI): OpenCode Go thinking models skip named `tool_choice` and retry a 400 whose body mentions it, so pinned `deepseek-flash` swarms no longer die after a late force-submit. Mid-turn OpenCode Go steer persists the live residual before receipt publish, so compact/sanitize no longer surface "history absent from this runner." Swarm engine errors include the stored `provider_body` (Go's thinking-mode reject) instead of only `http_status:400`. v0.9.476: prune toast no longer dies when the rail flickers `harness-project-switching` after SWR settle (the v0.9.475 main-push frontend-build miss). Push-to-main `tests` skips the pytest/frontend lottery when `merge^{tree}` already has a successful `tests` run; a conflict-changed tree still runs. v0.9.475: Server isolate no longer constructs `Session(qwen3-coder-30b)` when `harness.server` is already imported: xdist workers that left the eval catalog unreadable were erroring PTY setup with `no provider key available for pilot 'qwen3-coder-30b'`. Isolate now builds the offline `stub-oracle-v2` session and seeds `{}` at `PUPPETMASTER_MODELS_PATH`. v0.9.474: OpenCode Go mid-turn steer no longer dies with "history absent from this runner": live persist exports under `_busy_meta` so a stale SSE checkpoint cannot overwrite a compacted native transcript. Swarm Tracker hydrates finished aliases without a click or a live poll, so list titles come from `goal_preview` and degraded quality shows on the closed row; list prefetch does not clear a selected stale inspection. Recover rejects a stale `expected_turn_id` when a turn is live; `admit_front` reorders an already-queued id. v0.9.473 audit haul: `list_dir` survives dangling symlinks; write/edit restore prior bytes when post-replace verify fails; missing hook/session rename ids return 404; env bools and budget parse case-insensitively; doctor session dump is ceiling-clamped; Swarm Tracker titles a hydrated alias from `goal_preview`, keeps degraded quality sticky, and scopes `invalid_metadata` off the detail banner. Pilots get `cancel_job` and `job_findings`; peek/command overflow is head+tail; browser tools may open loopback without opening web_fetch SSRF; `run_swarm` accepts `prior_findings`; CodeGraph auto-inject keys off working context and no longer claims verbatim on-disk. List-lane operator facts still omit raw goals (privacy). v0.9.472: Swarm Tracker grouping is monotonic: a job whose PM row or 4s-hydrated detail already reported a terminal lifecycle stays under Finished even when the list row goes stale or the local alias lags, so expanding neighbours no longer snaps it back to Active and completed swarms move to Finished as soon as the detail sees it. The Finished header actually collapses (the row display class flips with the hidden attribute; Tailwind's .flex outranked preflight [hidden]). A finished detail read that lost the single-flight store re-arms at once instead of waiting a 4s slot. v0.9.471: A background job that finishes without ever delivering a result card (an interrupted `run_parallel` worker, a cancelled implement) no longer holds Still working… forever: the swarm poll gives the exact-result recovery drain a bounded number of empty passes, then confirms the job terminal, settles its pending pill as ended, and stops polling. v0.9.470: A failed list-lane metadata read (5xx, store hiccup, view regeneration) no longer stamps every cached job detail with an error: details go stale, rosters stay visible, and the alias and PM-row loops re-read on a 4s cadence until fresh. Submit still pins on the first paint even if trackpad inertia flips the release latch before that rAF; the second paint honors a real post-submit wheel-up. Hidden-tab typewriter timeouts convert back to rAF when the tab is shown. Boot persist writes `carry_price_source`. v0.9.469: Swarm Tracker follows a live job: the canonical alias re-hydrates on list revision or every 4s instead of freezing on its first read; a transient store lock (`read_snapshot_unavailable`), a mid-read revision advance, or a re-read expert race keeps the last roster visibly stale instead of blanking the card; the card body no longer repeats the whole instruction as a title. v0.9.468: Hidden-tab streams keep draining on a 33ms timeout instead of parking a rAF; an empty typewriter buffer stops the pump until the next delta; submit-pin honors a user scroll gesture; stdio MCP `start()` reaps the child if initialize times out; HTTP MCP SSE picks the last JSON-RPC result, not a trailing progress notification. Runtime pin: `puppetmaster-ai==1.27.13`.
...
| **Puppetmaster delegation** | `run_swarm`, `run_implement`, and `run_parallel` run bounded workers as durable jobs. Inspect recorded attempts, acceptance evidence, uncertain outcomes, and measured or estimated consumption. Marionette pins `puppetmaster-ai==1.27.13`; the installer and self-update use the same version. |
