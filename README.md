...
stdlib-only backend (urllib + sqlite); `puppetmaster-ai==1.27.9` is the one
real dependency the installer puts in the venv.

> Status: v0.9.457, deliberately pre-1.0. Failed swarms cannot inherit another job's findings: empty job_id snapshots refuse a multi-job last-job bind, unstamped findings are dropped from delivery, and the pilot digest leads with a job_id/status/findings bind. Swarm Tracker is compact cards plus inspect popup and Puppetmaster board pop-out again (not an iframe dump). Settings Reply output cap / token-budget floors stay. Pin `puppetmaster-ai==1.27.9` (OpenCode Go `x-opencode-session`; inline worker SystemExit cannot kill the host). Implement/parallel auto-route fail-closes to Settings Models toggles (toggled-off glm-5.2 cannot win); relocating or opening a repo mints the Puppetmaster project store so Jobs stays live; CodeGraph index files are not seeded into implement worktrees; DeepSeek OpenCode Go pins accept `opencode-go:` / `agentic/opencode/` aliases. Pin `puppetmaster-ai==1.27.9` (Darwin sqlite `source changed` after lock-open ctime; Settings Reply output cap for HARNESS_MAX_TOKENS). Pin `puppetmaster-ai==1.27.9` (Jobs-rail / dashboard Runs list shows human goals instead of Marionette provenance JSON). Composer always keeps the 64px Cursor-like clearance above the dock (including during Investigating / live turns). Jobs embeds hold the selected PM host across soft metadata refreshes so the pane does not snap back to the list UI. Cold session hops stay quiet (no empty dim flash). Pins `puppetmaster-ai==1.27.9` so provenance JSON labels are never the dashboard headline. Jobs list keeps the last observation page while metadata refreshes (no empty-state blink); dashboard embeds require a durable job_… id and fail fast instead of dashboard_failed_to_start on local-swarm aliases; conversation header brand truncates cleanly beside Retry/status when the window is narrow. Live streaming no longer chases the idle mid-pane seating reserve — overflow-anchor and stick-to-bottom stay on the same bottom line while tokens land, and the 64px composer gap is no longer zeroed during a live turn. Jobs rail hosts every Puppetmaster hire via `?job=&embed=1` (adapters are transports; no native strip). A sealed assistant finale no longer sits beside a still-streaming copy of the same text during the live turn; reload already collapsed those rows, and the live feed now does too. Opening another project no longer persists an empty transcript for the outgoing session across deferred cold-attach. Short and empty session transcripts stay top-aligned in the chat column with reserved space above the composer so greeting and live-tail rows are not glued under the dock; stick-to-bottom still follows long streams via overflow-anchor and scroll-padding-bottom. Pilot stream deltas accumulate verbatim so a later `###` heading or `**` bold marker is not treated as a replay of the opening chunk; a streamed final owns the trailing sealed bubble even when mid-stream glue had diverged, which was painting one clean summary and one mangled copy under a single PILOT label. Pins `puppetmaster-ai==1.27.9`. Fix late investigation card hoisting so it only targets the true trailing finale of a turn, preventing intermediate pre-tool assistant narration from being pulled down into the final answer as duplicate stacked bubbles. Completions and Responses stream snapshot deduplication for Luna/Codex/OpenRouter so PILOT messages do not double sentences or paragraphs. Passing verify commands (`npm test`, typecheck, lint, build) complete the in-progress Validate composer TODO; chat claims still do not write the checklist. The collapsed Swarm Tracker icon no longer shows an idle hollow holder — only the live blink. Session hop drops a foreign queueList instead of painting a pending-refresh error, and Swarm Tracker restores the last observation page instead of walking startup history again. Composer chrome is TODO + Puppetmaster/Terminal only — no observed-job dump and no Saved inputs accordion. Swarm Tracker hides Command rows and wave parents; expanded cards stay compact. Worker chevrons expand the instruction write-up in the list, not a popup. Idle ticks refresh live work only; Next page still walks history. Jobs list also hides commands. Applied worker patches no longer paint "I made this worse"; only an explicit failed acceptance blocks another paid swarm. Installed releases use a dedicated, revision-pinned runtime checkout so local work in an older checkout cannot block startup. Desktop windows keep inline panels; compact navigation and bounded drawers start below 640 CSS pixels. The Panels drawer includes its own Add panel chooser and accessible card controls. Fork is available from each session’s context menu. Empty legacy prompt queues no longer show a recovery warning.
...
| **Puppetmaster delegation** | `run_swarm`, `run_implement`, and `run_parallel` run bounded workers as durable jobs. Inspect recorded attempts, acceptance evidence, uncertain outcomes, and measured or estimated consumption. Marionette pins `puppetmaster-ai==1.27.9`; the installer and self-update use the same version. |
