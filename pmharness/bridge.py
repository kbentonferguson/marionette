...
def _analysis_instruction(goal: str, repo_cwd: str, role: str,
    ...
    if browser:
        ...
        return (
            f"{goal}{lens_line}{criteria_line}\n\nYou have a real Chrome browser "
            f"(headed when the harness published PM_BROWSER_HEADED; otherwise "
            f"headless). Use the browser tools to complete this: "
            f"browser_navigate(url) to open a page, then browser_snapshot() to "
            f"list interactable elements with @e-style refs, browser_get_text() "
            f"for the readable page text, and "
            f"browser_click/browser_type/browser_scroll/browser_back as needed. "
            f"If a login or Cloudflare wall appears, call browser_auth_handoff "
            f"and wait for the user to finish in the visible window -- never type "
            f"passwords and never paste cookies into findings. "
