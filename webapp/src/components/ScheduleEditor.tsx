...
export default function ScheduleEditor({ initial, pending, onSave, onCancel }: {
  ...
  return <form aria-label={initial ? "Edit schedule" : "New schedule"} className="space-y-3 rounded border border-edge bg-panel p-3"
    ...
    <fieldset disabled={pending} className="space-y-3">
      ...
      <label className="block text-sm">Cron expression<input required className={`${inputClass} font-mono`} aria-describedby="schedule-cron-help" value={draft.cron} onChange={e => setDraft({ ...draft, cron: e.target.value })} /></label>
      ...
      <label className="block text-sm">Missed runs<select className={inputClass} value={draft.missed_policy} onChange={e => setDraft({ ...draft, missed_policy: e.target.value })}>
      ...
      <details><summary className="min-h-11 cursor-pointer py-2 text-sm focus-visible:outline focus-visible:outline-accent">Run limits and continuity</summary>
        <div className="space-y-3">
          <p className="text-sm text-muted">Zero uses the governor default. Credentials and unspecified driver settings resolve on the daemon host.</p>
          ...
          <label className="block text-sm">Driver<input className={inputClass} value={draft.driver} onChange={e => setDraft({ ...draft, driver: e.target.value })} /></label>
